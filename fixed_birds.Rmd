...
knb_files <- list.files(here::here("knb-lter-cap_256_9"), pattern = "*.csv")
...
for (i in seq_along(knb_files)) {
  assign(knb_files[i] %>% 
           str_remove("52_pp52_") %>% 
           str_remove(".csv"), 
         read_csv(here::here("knb-lter-cap_256_9", 
                       knb_files[i])))
}
...
```{r, include = FALSE}
birds_df <-
  left_join(birds_1, taxalist_1, by = "species_id")
...
```{r}
ggplot(data = bird_totals_by_park, 
       aes(x = total_count, y = park_name)) +
  geom_col() +
...
```{r}
parks_plot <- ggplot(data = selected_parks, aes(
  y = common_name, x = total_count)) +
  geom_col() +
...
```{r}
ggsave("bird_counts.jpg", parks_plot, height = 6, width = 12)
```
